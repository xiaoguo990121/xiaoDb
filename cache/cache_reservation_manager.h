...
namespace XIAODB_NAMESPACE
{
    ...
    public:
        ...
        virtual Status UpdateCacheReservation(std::size_t new_memory_used) = 0; // 用于更新缓存预留，根据新的内存使用量来调整缓存空间
        ...
        virtual Status UpdateCacheReservation(std::size_t memory_used_delta, // 根据内存使用量的变化 memory_used_delta 和一个布尔值 increase 来更新缓存预留。如果 increase 为 true，则增加缓存预留；如果为 false，则减少缓存预留。
                                              bool increase) = 0;
        virtual Status MakeCacheReservation( // 用于预留增量的缓存空间。incremental_memory_used 表示需要额外预留的内存量，handle 是一个指向 std::unique_ptr<CacheReservationManager::CacheReservationHandle> 的指针，用于返回一个管理缓存预留生命周期的句柄。
            std::size_t incremental_memory_used,
            std::unique_ptr<CacheReservationManager::CacheReservationHandle>
                *handle) = 0;
        virtual std::size_t GetTotalReservedCacheSize() = 0; // 用于获取当前总共预留的缓存大小。
        virtual std::size_t GetTotalMemoryUsed() = 0;        // 用于获取当前总共使用的内存量
    };
