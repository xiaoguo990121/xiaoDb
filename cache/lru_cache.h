...
namespace XIAODB_NAMESPACE
{
    namespace lru_cache
    {
        ...
        // 缓存条目的状态
        // 1. 条目被外部引用，并且在哈希表中但不在LRU链表中（不能被驱逐）
        // 2. 条目没有被外部引用，但在哈希表中，并且在LRU链表中（可以被驱逐）
        // 3. 条目被外部用，但不在哈希表和LRU链表中，引用计数为0时条目将被释放
        struct LRUHandle : public Cache::Handle
        {
            Cache::ObjectPtr value; // 缓存条目的实际数据
            const Cache::CacheItemHelper *helper;
            LRUHandle *next_hash;
            LRUHandle *next;
            LRUHandle *prev;
            size_t total_charge; // 表示该缓存条目占用的总内存
            size_t key_length;
            // The hash of key(). Used for fast sharding and comparisons.
            uint32_t hash;
            // The number of external refs to this entry. The cache itself is not counted.
            uint32_t refs; // 表示外部引用该缓存条目的数量

            // Mutable flags - access controlled by mutex
            // The m_ and M_ prefixes (an im_ and IM_ later) are to hopefully avoid
            // checking an M_ flag on im_flags or an IM_ flag on m_flags.
            uint8_t m_flags; // 可变标志，包括该条目是否在缓存中，是否已被访问，是否在高优先级或低优先级池中
            enum MFlags : uint8_t
            {
            ...
            uint8_t im_flags; // 不可变标志，包括该条目是否为高优先级、低优先级或者是否是独立条目
            ...
            // Calculate the memory usage by metadata.计算缓存项的总费用，包括传入的 charge 和元数据费用。
        ...
        // table implementations in some of compiler/runtime combinations
        ...
        private:
            ...
            // The table consists of an array of bucketss where each bucket is
        ...
        private:
            ...
            // the LRU list. Older items are evicated as nucessary. Frees `item` on
            // non-OK status.
            ...
            double high_pri_pool_ratio_; // 高优先级池的容量占总容量的比例
        ...
        public:
            ...
            void ApplyToHandle(
                ...
                const std::function<void(const Slice &key, ObjectPtr obj, size_t charge,
                                         const CacheItemHelper *helper)> &callback) override;
            // Retrieves number of elements in LRU, for unit test purpose only.
            size_t TEST_GetLRUSize();
            // Retrieves high pri pool ratio.
            double GetHighPriPoolRatio();
