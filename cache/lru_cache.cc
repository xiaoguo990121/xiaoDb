...
#include "monitoring/perf_context_impl.h"
    ...
    namespace lru_cache
    {
        ...
        void LRUHandleTable::Resize()
        {
            if (length_bits_ >= max_length_bits_)
            {
                ...
                // we would allocate more address but only the same number would be used.
                return;
            }
            if (length_bits_ >= 31)
            {
        ...
        {
            return SumOverShards([](LRUCacheShard &cs)
                                 { return cs.TEST_GetLRUSize(); });
        }
