...
#include "xiaodb/functor_wrapper.h"
...
#define XIAODB_PRINTF_FORMAT_ATTR(format_param, dots_param) \
...
namespace XIAODB_NAMESPACE
{
    ...
    public:
        ...
        //
        // The result of Default() belongs to XIAOdb and must never be deleted.
        static Env *Default();
        ...
        // used in XIAODB internally to update thread status and supports
        // GetThreadList().
...
#undef GetFreeSpace
    ...
    public:
        ...
        // the future. Currently, XIAODB only generates crc32c based checksum for
    ...
    public:
        ...
        // Set to INFO_LEVEL when XIAODB is compiled in release mode, and
    ...
    void Log(const InfoLogLevel log_level, const std::shared_ptr<Logger> &info_log,
             const char *format, ...) XIAODB_PRINTF_FORMAT_ATTR(3, 4);

    // a set of log functions with different log levels.
    void Header(const std::shared_ptr<Logger> &info_log, const char *format, ...)
        XIAODB_PRINTF_FORMAT_ATTR(2, 3);
    ...
    // XIAODB_NAMESPACE::SequentialFileWrapper {
    //  public:
    //   MySequentialFileWrapper(XIAODB_NAMESPACE::SequentialFile* target):
    //     XIAODB_NAMESPACE::SequentialFileWrapper(target) {}
    //   Status Read(size_t n, Slice* result, char* scratch) override {
    //     cout << "Doing a read of size " << n << "!" << endl;
    //     return XIAODB_NAMESPACE::SequentialFileWrapper::Read(n, result,
    //     scratch);
    //   }
    ...
    //     XIAOdb class. Unless you actually want to override the behavior.
    //     (And unless XIAOdb people forgot to update the *Wrapper class.)
